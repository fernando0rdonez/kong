...
const SPRITES = [
  {
    key: "player-walk",
    prompt: `A reference sprite sheet for a 2D game, 4 frames of the exact same friendly cartoon pirate-adventurer kid character walking, red shirt, yellow bandana and belt, big round friendly eyes, chibi proportions, side view walking to the right, arranged in a 2x2 grid (2 columns, 2 rows) reading left-to-right then top-to-bottom as a walk cycle, identical character design/proportions/colors/scale in every frame, character centered in each cell and aligned to the same ground baseline, no visible grid lines or borders between cells, no overlap between frames, ${STYLE_SUFFIX}`,
    apiSize: "1024x1024",
    width: 128,
    height: 128,
    frameWidth: 64,
  ...
  {
    key: "player-jump",
    prompt: `The exact same friendly cartoon pirate-adventurer kid character as a walk-cycle reference (red shirt, yellow bandana and belt, big round friendly eyes, chibi proportions), now in a mid-air jump pose with knees tucked up and arms raised, side view facing right, ${STYLE_SUFFIX}`,
    apiSize: "1024x1024",
    width: 64,
    height: 64,
    trim: true,
  ...
  {
    key: "enemy-walk",
    prompt: `A reference sprite sheet for a 2D game, 4 frames of the exact same grumpy cartoon pirate crab enemy walking/shuffling sideways, round spiky purple shell with a darker purple outline and small spikes on top, angry furrowed eyebrows over big round eyes, a raised snapping pincer, a tiny red bandana tied on top of the shell, side view facing right, arranged in a 2x2 grid (2 columns, 2 rows) reading left-to-right then top-to-bottom as a walk cycle, identical character design/proportions/colors/scale in every frame, character centered in each cell and aligned to the same ground baseline, no visible grid lines or borders between cells, no overlap between frames, ${STYLE_SUFFIX}`,
    apiSize: "1024x1024",
    width: 120,
    height: 120,
    frameWidth: 60,
  ...
  {
    key: "door-ajar",
    prompt: `The exact same green wooden treasure door archway structure on a tropical island, but only barely cracked open a small amount (partway between fully closed and fully open, mostly still shut with just a thin sliver of warm golden glow showing), filling almost the entire frame with minimal empty space around it, front view, tall portrait orientation, ${STYLE_SUFFIX}`,
    apiSize: "1024x1536",
    width: 140,
    height: 170,
    fit: "fill",
